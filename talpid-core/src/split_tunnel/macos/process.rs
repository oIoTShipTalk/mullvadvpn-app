...
mod test {
    ...
    use talpid_platform_metadata::MacosVersion;
    use tokio::io::{simplex, AsyncRead, AsyncWriteExt};
    ...
    async fn test_parse_logger_status_responsive() {
        ...
        assert!(
            need_fda,
            "expected 'true' when ES_NEW_CLIENT_RESULT_ERR_NOT_PERMITTED was eventually printed to stderr"
        );
    ...
    async fn test_parse_logger_status_hogged() {
        ...
        assert!(
            !need_fda,
            "expected 'false' when nothing was ever printed to stdout or stderr"
        );
    }
}
