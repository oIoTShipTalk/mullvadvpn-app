    ...
    AccountDelegate
{
  ...
  private splitTunneling?: ISplitTunnelingAppListRetriever | undefined
  ...
  private registerIpcListeners() {
    ...
    IpcMainEventChannel.splitTunneling.handleAddApplication(async (application) => {
      ...
      if (typeof application === 'string') {
        const executablePath = await this.splitTunneling!.resolveExecutablePath(application);
  ...
  private allowDevelopmentRequest(url: string): boolean {
    if (process.env.NODE_ENV === 'development') {
      ...
      return (
        isViteDevServerRequest(url) ||
        isDevtoolsRequest(url)
      );
    }
